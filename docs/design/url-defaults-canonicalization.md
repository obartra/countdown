...
## Summary
Editor and viewer share one definition of defaults so canonical URLs omit default-valued params while rendering identically. This is implemented via `src/countdownUrl.ts` (canonicalization/helpers) and applied on initial load in `src/main.tsx`.
...
- Canonicalization implemented in `src/countdownUrl.ts`: normalizes text, trims, normalizes colors (hex/name), omits defaults (complete text, default theme pair) and respects partial overrides; merges extras unchanged.  
- `src/main.tsx` canonicalizes the URL on load for both viewer and editor paths; query overrides patch over stored payloads for `/v/:slug`.  
- Editor (`src/EditPage.tsx`) uses canonical params for publish/share; inputs keep user-entered variants but URL omits defaults.  
- Viewer (`src/App.tsx`, `src/hooks/useCountdownViewModel.ts`) renders from canonical params; colors resolved via `deriveColors` + themes. Default complete text shared via `DEFAULT_COMPLETE_TEXT`.
...
Implemented: shared utility (`src/countdownUrl.ts`) centralizes defaults, canonicalization, and serialization; editor keeps display vs. canonical split; viewer/editor share the same canonical parse/serialize rules.
